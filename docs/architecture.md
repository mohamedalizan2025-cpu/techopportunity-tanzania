...
### 12.20 Milestone 14 — published-record management + public trust cleanup (2026-08-30)

Milestone 13 proved a public trust defect: loudly-titled test artifacts
("PRODUCTION LINK TEST — DELETE ME", "REGRESSION Alpha", "REGRESSION
Bravo", "hack", "hackkka", a Swahili news headline) are live on the
homepage and category filters, while moderation only walks the pending
queue — so no safe staff path existed to hide an already-published
record. This milestone builds that control surface and its safety
guarantees; it performs NO cleanup itself.

**Surface (`/published-management`, deliberately per-record)**

- Staff-only list of the live published set: title, category (existing
  `categoryLabel` layer), organization, source name (or an explicit
  "none recorded (manually entered)"), publication date, "Status ·
  publicly visible", a link to the public page, and one unpublish action
  per row with a required confirmation step.
- Built inside the SAME authorization boundary as moderation:
  `getModerationAccess()` → `redirect("/login?next=…")` when
  unauthenticated, "Access restricted" + sign-out when non-staff. No
  second auth system, no public mutation route, `robots: noindex`.
- Reuses `OPPORTUNITY_SELECT` + `mapOpportunityRow`, the same explicit
  500-row cap and the same deterministic `created_at asc, id asc`
  ordering as the queue.
- Explicitly NOT built: dashboards, analytics, bulk selection, bulk
  deletion, new moderation state machines, new infrastructure.

**Unpublish semantics — existing status, zero DDL**

- `published → rejected`: the enum value the app already treats as
  "stays hidden from the public site". No new status invented, no
  migration touched, no row deleted.
- The write is structurally `{ status: "rejected" }` and nothing else
  (`unpublishUpdatePayload`), so source id/url, discovery method,
  discovered_at, title, description and every evidence field are
  unreachable through this path. The public detail page then 404s via
  the pre-existing published-only read.
- Honest limitation recorded instead of worked around: migration 0003
  constrains `opportunity_enrichments.field` to
  `('venue_name','address','city','region','deadline')`, so a
  status-change audit row CANNOT be written without DDL. None is
  attempted and none is claimed — reported as an owner gate, not applied.
- A rejected row does not re-enter the pending queue, and the UI says so
  plainly rather than promising a re-publish path that does not exist.

**Single-record safety**

- Pure, pre-query gates: `parseUnpublishRequest` (exact UUID target plus
  the fixed confirmation token; a malformed id is refused before any
  database contact), `evaluateUnpublishPermission` (staff identity),
  `evaluateUnpublishTarget` (status must still be `published`).
- Conditional update `.eq("id", id).eq("status", "published")` with
  `.select("id,title")`: a record changed meanwhile yields zero rows and
  the action refuses rather than clobbering — double-click and
  concurrent staff action are safe by construction.
- Arbitrary transitions are impossible: exactly one payload, targeting
  exactly one value, for exactly one id, staff-only.

**Client/server boundary (caught by the build, not by review)**

The first build attempt failed on a genuine defect this milestone
introduced: a client component imported a value from a server-only data
module, pulling `next/headers` into the browser bundle. The shared
confirmation token now lives in the client-safe
`lib/staff-form-state.ts`. The second attempt returned to the known
Turbopack os error 5 — environment-blocked, not redesigned.

**Battery**: **296/296** tests (253 prior + 43 new
`tests/published-management.test.ts`, mapped 1:1 onto the ten required
safety guarantees), lint clean, tsc clean (standard + ci-check).
GitHub Discovery sync: still runs #1–#3, all pre-fix failures — CI
...
Milestone 15 was meant to be a human session, not an engineering pass. It
is recorded here because it measured a ceiling that changes what the next
milestone must be. **Architecture verdict unchanged: sound, closed.** No
code was written, no data was modified, no migration was touched.

**Session status: not executable in this environment (no simulation)**

- `.env.local` exposes only the three Supabase variables — no staff
  credential of any kind, and no client session exists.
- `profiles` holds exactly **one** row with `role='moderator'`, which both
  `is_staff()` and `getModerationAccess()` accept (verified: the app's
  `['moderator','admin']` check matches the SQL function's
  `role in ('admin','moderator')`). So the account already exists; only
  the interactive password/browser session is out of reach here.
- Reported as required: *Staff session unavailable; implementation
  verified by code, tests, and public authentication boundary.*
- A BEFORE baseline was armed for the owner's cleanup: all five artifact
  detail URLs return 200, and `REGRESSION Alpha/Bravo`, `hackkka`,
  `hack`, `PRODUCTION LINK TEST` and the `WAZIRI…` headline are all
  present on `/`, `/?category=hackathon|competition|fellowship|scholarship`
  and `/?sort=newest`. Pending and rejected details are 404.

**The measured ceiling: no deadline evidence reaches the product**

- **198 / 198** pending rows carry `deadline = NULL`.
- A read-only discovery dry-run over the live registry: 18 sources, 20
  fetches, 2 failures, **147 valid candidates, `withDeadline = 0`,
  `withLocation = 0`, `withNeither = 147`**.
- Of the 10 published rows, exactly one legitimate record has a deadline
  at all — `ERASMUS GLOBAL CALL FOR APPLICATIONS`, `2026-07-15`, already
  past. Derived freshness: 1 active (a test artifact), 2 expired, 7
  unknown ⇒ **0 trustworthy provably-live public opportunities**.
- This is NOT a persistence bug and NOT a defect: `buildPendingRow` does
  write `deadline`, and the extractor deliberately refuses `startDate`/
  `endDate`/`pubDate` as deadlines (fixture-tested). Tanzanian
  institutional pages simply carry no JSON-LD deadline field, so the
  honesty policy correctly yields NULL. The consequence is arithmetic:
  publishing any part of the backlog today produces "No deadline listed"
  cards, so moderator effort cannot by itself raise the KPI.
- Therefore, after the owner hides the five artifacts, the public site
  shows 5 rows — 4 category `other`, 1 expired-scholarship. Public trust
  improves; inventory does not. Both facts belong in the next decision.

**Corrections to earlier handoff claims (evidence, not theory)**

- "Moderator throughput is THE binding constraint" is now half-refuted:
  it is A constraint; the deadline-evidence ceiling is upstream of it.
- Registry health: 29 sources / 18 active, all with `last_success_at`,
  none with `last_error`. Today's dry-run failed to fetch 2 of 18 (Bank
  of Tanzania, Youth of UNATA — previously the best-yield source), which
  is drift to watch, not a code regression.
- Latent owner-gate yield, measured without applying anything: 5 of the
  147 live candidates propose `admissions` (4) or `jobs` (1), which
  `runner.ts` counts as `categorySkipped` because no seed row exists.
  That is the real, bounded expected recovery from 0004 + 0010 — worth
  applying, but not a flood of new inventory.

**Battery**: 296/296, lint clean, tsc clean (standard + ci-check). One
build attempt: Turbopack `os error 5` at pooled-process spawn AFTER all
code gates passed — environment-blocked, and independently answered by
the production Vercel build succeeding with `/published-management` live.
GitHub: still runs #1–#3; run #3's steps are install ✓ tests ✓
...
| 2026-08-30 | Milestone 15 (§12.21): no session simulated, no code written; measured that the KPI is capped upstream of moderation — 198/198 pending and 147/147 live candidates carry no deadline evidence, and 0 legitimate published rows have a future deadline | Derived freshness from the production lifecycle function against the live DB and a read-only dry-run, not from estimates; the honest next milestone is deadline-evidence capture feasibility, because no amount of moderator effort can publish a provably-live opportunity today |
| 2026-08-30 | Milestone 14 (§12.20): staff-only `/published-management` with per-record unpublish implemented as an EXISTING-status transition (published → rejected), pure pre-query gates, conditional update, zero DDL, zero deletion; no cleanup executed | M13 proved test artifacts are publicly visible with no safe path to hide them; this is the smallest control surface that closes the trust defect without inventing a status, weakening RLS or touching provenance — bulk deletion stays an owner decision |
...
| 2026-09-01 | Milestone 22: add a positive technology/research/innovation scope contract, exclude admissions, require explicit opportunity evidence for institutional-source ambiguity, treat prior-year-only cohorts as stale, recognize exact WBG member-country eligibility, and prevent cohort years/single items from triggering roundup expansion; no schema, registry, schedule, AI, or existing-row change | Run 8 inserted five rows, of which three were false children caused by parsing `2027/2028` as a roundup count. A same-page replay measured 124 pre-dedupe survivors/121 eligibility-unknown before source-aware filtering and 8/5 after all M22 gates, with 18/18 sources and 10/10 detail fetches successful. Product fit is now positive-evidence-driven rather than inferred from generic opportunity families; historical cleanup and production proof remain separate owner gates |
| 2026-09-01 | Milestone 23: establish a deterministic milestone verification contract, credential-free push/PR CI, static architecture/security boundary checks, change classification, and discovery-sensitive push verification; retain the daily discovery schedule and existing pending-only worker | M22's exact SHA had zero GitHub Actions runs, so its production gap is recorded rather than fabricated. Automatic exact-head evidence removes repetitive manual dispatch while production claims remain SHA-correlated; one successful run still cannot prove six-hour repeatability. No schema, registry, source, discovery semantics, AI, or existing data changed |
...
| 2026-09-14 | Production promotion of exact pending-rejection capability `71f4a33` and migration 0016; no live moderation action used for proof | Fresh pre-change recovery plus full ID comparison proved only expected scheduled-discovery growth; migration committed with zero row changes and exact 0015 definitions intact; exact-SHA verification, Discovery sync (`insertedPending: 0`), schedule-health, and deadline-evaluation workflows all succeeded; post-promotion comparison proved zero added/removed/changed rows and both real pending records untouched |
| 2026-09-14 | Attributable two-record resolution through the new reason-bearing path | Both real pending records rejected one at a time through the authenticated Moderator UI with exact verbatim reasons (164/177 chars), each fully verified before the next was touched. Full-column comparison shows only the two rows changed; all references preserved; production is 254 pending / 8 published / 21 rejected with 2 status audits |
