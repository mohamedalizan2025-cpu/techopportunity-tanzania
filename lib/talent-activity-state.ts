 ...
 *
 * `saved` stays the private bookmark in `saved_opportunities` (untouched).
 * This module owns the three application-progress states tracked in the
 * owner-gated `talent_opportunity_activity` table (migration 0019):
 * one funnel row per user+opportunity, owner-mutable between states.
 *
 ...
 * answer different questions ("revisit later?" vs "where am I?").
 */
