...
export function OpportunityFilters({
  ...
  const hasLocations =
    locations.cities.length > 0 || locations.regions.length > 0;
  ...
  const activeChips = [
    activeQuery
      ? {
          ...
          href: buildHref(activeCategory, withoutQuerySort, {
            ...shared,
            q: null,
          }),
        }
    ...
    (chip): chip is { key: string; label: string; href: string } =>
      chip !== null,
  );
    ...
    >
      ...
      >
        ...
        <div className="flex w-full gap-2">
          <div className="relative min-w-0 flex-1">
            <UiIcon
              name="search"
              className="pointer-events-none absolute left-4 top-4 text-[var(--muted)]"
            />
            <input
        ...
        <p className="mt-2 text-xs leading-5 text-[var(--subtle)]">
          Try “AI”, “fellowship” or the name of an organization.
        </p>
      </form>
      ...
      >
        <summary>
          ...
          <UiIcon name="chevron" className="disclosure-chevron" />
        </summary>
        <form
          action="/#opportunities"
          method="get"
          aria-label="Opportunity filters"
          className="flex w-full flex-col gap-3 pb-4 pt-1 sm:flex-row sm:flex-wrap sm:items-end"
        >
          <HiddenState
            category={activeCategory}
            sort={activeSort}
